...
def calcApertureUnsymmetrical(opt, aper):
    edgeUnit = 2 * np.sin(np.pi / aper.sideNumber)
    ...
    aperShape = np.array([(np.sin(2 * np.pi * ii / aper.sideNumber), np.cos(
        2 * np.pi * ii / aper.sideNumber)) for ii in range(aper.sideNumber)])
    xPOS = [(aperShape[ii, 0]) * xMags[ii] + round(opt.Npixel / 2)
            for ii in range(aper.sideNumber)]
    yPOS = [(aperShape[ii, 1]) * yMags[ii] + round(opt.Npixel / 2)
            for ii in range(aper.sideNumber)]
...
def calcApertureUnsymmetricalRandom(opt, aper):
    edgeUnit = 2 * np.sin(np.pi / aper.sideNumber)
    xMags = [length / opt.dx / edgeUnit for length in aper.sideLengths]
    yMags = [length / opt.dx / edgeUnit for length in aper.sideLengths]
    aperShape = Path(np.array([(np.sin(2 * np.pi * ii / aper.sideNumber), np.cos(
        2 * np.pi * ii / aper.sideNumber)) for ii in range(aper.sideNumber)]))
    xPOS = [(aperShape.vertices[ii, 0]) * xMags[ii] + round(opt.Npixel / 2)
            for ii in range(aper.sideNumber)]
    yPOS = [(aperShape.vertices[ii, 1]) * yMags[ii] + round(opt.Npixel / 2)
            for ii in range(aper.sideNumber)]
