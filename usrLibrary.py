...
def calcAuNP(opt, par):
    ...
    singleNP3dcrop = (par.diameter / 2)**2 >= (Xw3d**2 + Yw3d**2 + Zw3d**2)                 # 3D single particle (in minNpixel x minNpixel x minNpixel)
    # calcuate 2d projected image
    singleNP2dcrop = np.squeeze(opt.dx * singleNP3dcrop.sum(axis = 2))                      # 2D projected single particle (in minNpixel x minNpixel)
    singleNP2dpad  = np.pad(singleNP2dcrop, int((opt.Npixel - minNpixel) / 2), "constant")  # 2D projected single particle (in Npixel x Npixel)
    
    # apply absorption and phase parameter to the 2d projected image
    singleAuNPpad = np.exp(-opt.k * (par.beta_Au + 1j * par.delta_Au) * singleNP2dpad)      # 2D projected single particle (in Npixel x Npixel)
    singleAuNPcrop = np.exp(-opt.k * (par.beta_Au + 1j * par.delta_Au) * singleNP2dcrop)    # 2D projected single particle (in minNpixel x minNpixel)
    ...
    for iFrame in range(0, par.Nframe):
        ...
        for iNumber in range(0, par.number):
            ...
            smallNumber  = 50
            xTouchWindow = (xEffective < (singleAuNPcrop.shape[0] / 2 + smallNumber)) or (xEffective > (opt.Npixel - singleAuNPcrop.shape[0] / 2 - smallNumber))
            yTouchWindow = (yEffective < (singleAuNPcrop.shape[1] / 2 + smallNumber)) or (yEffective > (opt.Npixel - singleAuNPcrop.shape[1] / 2 - smallNumber))
            if xTouchWindow == 1 or yTouchWindow == 1: # method 1, slow but simple (shift the particle image)
                AuNPimg[:, :, iFrame] *= np.roll(np.roll(np.fft.fftshift(singleAuNPpad), int(np.round(NPpos[0, iNumber, iFrame] / opt.dx)), axis = 0), int(np.round(NPpos[1, iNumber, iFrame] / opt.dx)), axis = 1)
            else:
...
def calcAperture(opt, aper):
    ...
    aperShape = Path(np.array([(np.sin(2 * np.pi * ii / aper.sideNumber), np.cos(2 * np.pi * ii / aper.sideNumber)) for ii in range(aper.sideNumber)]))
